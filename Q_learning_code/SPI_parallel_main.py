...
def initQMatrices(game, delta, Profits, indexActions):
    ...
    for iAgent in range(game.numPlayers):
        for iReport in range(game.numiActions):
            den = np.count_nonzero(game.indexActions[:, iAgent] == iReport) * (1 - game.delta)
            Q[:, iReport, iAgent] = np.ma.array(
                (1 - game.eps) * game.Profits[0][:, iAgent] + game.eps * game.Profits[1][:, iAgent],
                mask=(game.indexActions[:, iAgent] != iReport)).sum() / den
    ...
    for iSession in range(game.numSessions):
        ...
        for iters in range(game.maxIters):
            ...
            for iPlayer in range(game.numPlayers):
                ...
                if random.uniform(0, 1) < 1 - game.eps:
                    profits = game.Profits[0, actionPrime, iPlayer]
                else:
                    profits = game.Profits[1, actionPrime, iPlayer]
                newq = oldq + game.alpha[iPlayer] * (profits + game.delta * maxVal[statePrime, iPlayer] - oldq)
...
def single_process(alpha):
    ...
    #c_list = [ 10000]
    for cl in c_list:
        df = pd.DataFrame(columns=['alpha','beta','total_sessions','converged_times','indexStrategie','indexConverge'])
        #alpha_list = np.linspace(0.025,0.25,100)
        beta_list = np.linspace(0.0005,0.00001,20)
        path = 'SPI_parallel_result'
        if not os.path.exists(path):
            os.mkdir(path)
        
        for beta in beta_list:
        
            spi = SPIModel(alpha = alpha, beta = beta)
            converged, indexStrategies,indexConverge = q_learning(spi,convergedtime= cl)
            #df = pd.DataFrame(columns=['alpha','beta','total_sessions','converged_times','indexStrategie','indexConverge'])
            df = df.append({'alpha':alpha,'beta':beta,'total_sessions':spi.numSessions,'converged_times':cl,'indexStrategie':indexStrategies,'indexConverge':indexConverge },ignore_index=True)
